...
(define (sum-odds a b)
  (filtered-accumulate + 0 (lambda (x) (* x x)) a (lambda (x) (+ x 1)) b odd?))

; Definite integral

(define (definite-integral f ax bx dx)
  (* (filtered-accumulate + 0 (lambda (x) (f (+ ax (/ (* x dx) 2)))) 1 (lambda (x) (+ x 1)) (/ (* 2 (- bx ax)) dx) odd?) dx))